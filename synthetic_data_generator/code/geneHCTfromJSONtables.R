...
# 2 JSON table + style
# 3 read data 
# 3.0 read table (generate n -> for loop)
# 3.1 generate flat DB table (all rows, but only specific data attributes specified in table)
# loop over n
# 3.2 generate specific flat table by filtering (statistics + selected data row and columns only from subset) 
# 3.2 assign row and column variable groups to prepare pivot of DB table
# 3.3 assign options of aggregate to prepare pivot of DB table

# DONE always balanced; 
# DONE with aggregation or not on rows and/or columns; 
# DONE explicit or implicit on rows;
# DONE with 1 to 3 levels on rows and/or columns, 
# DONE possibly asymmetric when we use tree-based semantic attributes.
# DONE: add global option to randomly shuffle within rows/col, shuffle across rows and columns, or transpose  rows and columns
# DONE: put row and col option outside row and col group in table.json
# DONE: improve csv rendering   
# DONE: rename columns of expL with a name1.name2 instead of codes to ease spliting and other manipulations (random shuffle) before row/col spliting
# DONE: asymmetric: flag to tell to sample at random for say Quarters differently for each year
# DONE:	rescale automatically to fit in one page of pdf
# DONE: 50-50% adding line boundaries
# DONE: check each replica is distinct by their attribute values (not only by their content values) USE A GLOBALE VAR TO STORE PARAM

# TODO: ADD ATTR NAMES + OPTION TO show/hide attribute names
# TODO: add a semantic name to the values in tables, like number of, salary, temperature ... to help questions

# DONE:	Have a signature for each table to not generate the same table twice
# DONE:	generate the JSON file corresponding to each table as a JSON file (check JSON format below, see Zan for questions)

# DONE - generating all semantic data as JSON

#### TODO - generate JSON to auto-generate below ~1000 tables

## DONE -- Problem with random seed, reinit with replica number so we can reproduce bugs
## DONE -- Problem with duplicated values if not enough decimal numbers for real values

## DONE -- vertical centering of row names when now border line, should align parent names to top
...
# # Levels	1 (flat), 2, 3
# Balanced	Yes, No --> YES ONLY
# Symmetric	Yes, No
# 
# Row nesting	3 combination
# # Levels	1 (no nesting), 2
# Balanced	Yes
# Symmetric	Yes, No
# 
# Aggregations	16 combinations
# Column Global Aggregation	Yes (Explicit), No
# Row Global Aggregation	Yes (Explicit), No
# Column Local Aggregation	Yes (Explicit), No
# Row Local Aggregation 	Yes (Explicit), No
# 
# Row Group Label (empty row)	2 combinations
# Row Group Label (empty row)	Yes, No
# 
# Split Header 	No
...
##### JSON FORMAT WITH Table characteristics as in the table annotator
# {"id": "189", 
#   "image_source": "static/images/189.jpg", 
#   "state": "labelled", 
#   "concern": false, 
#   "notes": "", 
#   "properties": {
...
#     "Balanced Multi Level Column": false, 
#     "Symmetric Multi Level Column": false, 
...
#     "Row Nesting": false, 
#     "Balanced Row Nesting": false, 
#     "Symmetric Row Nesting": false, 
...
#     "Split Header Cell": true, 
#     "Row Group Label": false}, 
#   "themes": []}
# 


# 3.4 assign style options
# 4 generate pivot table of DB table based on 3.x
# end
...
PDF_OK = FALSE
PNG_OK = FALSE # requires PDF_OK = TRUE to generate PNG
HTML_OK = TRUE


################ I/O #####################

inputFolder  = "./PARAMETERS/"
##outputFolder = "./TABLES_ALL/" 
outputFolder = "./TABLES_FOR_PROMPT_LLM/"
semanticsJSONfile = "semantics.json" # file containing all semantic data
##tablesJSONfile = "tablesToGenerate_all.json"       # file containing all table patterns to generate
tablesJSONfile = "tablesToGenerate_forPromptLLM.json"       # file containing all table patterns to generate
...
#for (itab in 242:242 ){ #length(allTablesJSON)){
# for (itab in 1:numTables){
# 
#   tableJSON = allTablesJSON[[itab]]
#   print(c(itab, tableJSON$name))
# 
# }
# break
# #     
...
for (itab in 1:numTables){
#for (itab in 1){  ## Disable see and clear all on top of this file before running
