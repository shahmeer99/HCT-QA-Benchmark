####################################################################################
####################################################################################
#### 2024_09_16 ONGOING: this file adding the JSON template for Q&A
####################################################################################
####################################################################################
...
#### DONE -- enable simplification of NLQ with nested attributes field: simplifyNested in NLquestionPattern templates
#### DONE -- result format should be: {A | B} || {C | D} where A and B on same row, C and D on another row...
#### DONE -- debug misisng iput and duplicate results
#### DONE -- generating the Q&A json annotation file

#### DO -- check correctness of annotation file (template 14 and 15?)
#### DO -- items in NLQ should preserve hierarchy top level to bottom level, and grouping by rows and columns to ease reading
#### DO -- ordering of results: should preserve table ordering
...
for (DATA_PREFIX in LIST_DATA_PREFIX){
  ...
  # DATA_PREFIX = "Weather_statistics"
  ...
  # DATA_SET = "set357_1"
  # DATA_FILE = "Weather_statistics_set150_5"
  ...
  # inputFolder  = "./TABLES_ALL/" 
  # outputFolder = "./QandA_ALL/"
  dataFolder   = "./PARAMETERS/" 
  ...
  for (indDataFile in numDataFiles){
    curDataFile = fileListToProcess[indDataFile]
    
  #for (indDataFile in 1:1){
  #  curDataFile = "Weather_statistics_set150_5"
    
    #curDataFile = "Number_of_graduations_set164_5"
    #curDataFile = "Number_of_students_set112_1"
    ...
    if (file.exists(DBcsvFile)){
      ...
      print(" TMP to get number of values for topK")
      print(sqlClauseTMP)
      print(sqlResultTMP)
